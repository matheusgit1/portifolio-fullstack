...
export default function Header({ children }: { children: React.ReactNode }) {
  ...
  async function getReposCount() {
    ...
    let data = JSON.parse(localStorage.getItem("repo_counts") || "0");
    let useravatar = JSON.parse(localStorage.getItem("github_avatar") || "0");
...
function ResumeViewer({ openResume }: { openResume: () => void }) {
  function dowloadCv(resume = "/CV/resume.pdf") {
